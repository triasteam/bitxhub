...
func (t *TssMgr) IsTssRoundDone(msgID string) (bool, error) {
	...
	return info.DoneIDLen == info.ParitiesIDLen-1, nil
}

func (t *TssMgr) SetTssRoundDone(msgID string, notParties bool) error {
	// if this tss node is not parties, ignore tss msg
	if notParties {
		info := &KeyRoundDoneInfo{ParitiesIDLen: 1, DoneIDLen: 0}
		t.keyRoundDone.Add(msgID, info)
		t.logger.WithFields(logrus.Fields{"info": info}).Debugf("set not Parties Tss Round done")
		return nil
	}
	val, ok := t.keyRoundDone.Get(msgID)
	if !ok {
	...
	info.DoneIDLen++
	t.logger.WithFields(logrus.Fields{"info": info}).Debugf("set Tss Round done")
	t.keyRoundDone.Add(msgID, info)
	return nil
}
